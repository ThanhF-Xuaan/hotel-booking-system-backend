...
@Schema(description = "DTO dùng để cấu hình loại phòng cho khách sạn (HotelRoomType) khi tạo mới")
public class HotelRoomTypeCreationRequest {

    @NotNull(message = "HOTEL_ROOM_TYPE_HOTEL_ID_NOT_NULL")
    @Schema(description = "ID của khách sạn", example = "1", requiredMode = Schema.RequiredMode.REQUIRED)
    Short hotelId;

    @NotNull(message = "HOTEL_ROOM_TYPE_ROOM_TYPE_ID_NOT_NULL")
    @Schema(description = "ID của loại phòng", example = "1", requiredMode = Schema.RequiredMode.REQUIRED)
    Short roomTypeId;
